...
describe("/v1/responses bridge", () => {
  test("toChatBody: instructions + items + tools", () => {
    ...
    expect(chat.tools).toEqual([
      { type: "function", function: { name: "ls", description: "list files", parameters: { type: "object" } } },
    ]);
  ...
  test("chatChunkToEvents: content stream → output_text.delta sequence", () => {
    ...
    expect(ev1.map((e) => e.type)).toEqual([
      "response.created",
  ...
  test("chatChunkToEvents: tool call deltas → function_call events", () => {
    const st = newState();
    const ev1 = chatChunkToEvents(
      { choices: [{ delta: { tool_calls: [{ index: 0, id: "call_9", function: { name: "ls", arguments: "" } }] } }] },
      st,
    );
