...
describe("command-code wrap", () => {
  ...
  test("paired tool call + result → tool-call/tool-result parts", () => {
    const out = wrap({
      model: "m",
      messages: [
        { role: "assistant", content: "checking", tool_calls: [{ id: "call_1", type: "function", function: { name: "ls", arguments: '{"dir":"."}' } }] },
    ...
    expect(messages[1]).toEqual({
      role: "tool",
      content: [{ type: "tool-result", toolCallId: "call_1", toolName: "ls", arguments: '{"dir":"."}', output: { type: "text", value: "a.txt" } }],
    });
  });
  ...
  test("unpaired tool call dropped", () => {
    const out = wrap({
      model: "m",
      messages: [{ role: "assistant", content: "", tool_calls: [{ id: "call_x", type: "function", function: { name: "ls", arguments: "{}" } }] }],
    });
    expect(out.params.messages).toEqual([]);
  });

  test("reserved tool_search renamed consistently both ways", () => {
    const wrapped = wrapCommandCode({ model: "m", messages: [], tools: [{ type: "function", function: { name: "tool_search", parameters: {} } }] });
    expect((wrapped.body.params as Record<string, any>).tools as Record<string, any>[]).toHaveLength(1);
    expect(((wrapped.body.params as Record<string, any>).tools as Record<string, any>[])[0].name).toBe("troy_tool_search");
  ...
  test("tool_search round trip: call + result both wire-renamed", () => {
    const wrapped = wrapCommandCode({
      model: "m",
      messages: [
        { role: "assistant", content: "", tool_calls: [{ id: "call_1", type: "function", function: { name: "tool_search", arguments: '{"q":"x"}' } }] },
  ...
  test("reasoning passthrough fields reach params", () => {
    const out = wrap({ model: "m", messages: [], reasoning_effort: "high", thinking: { type: "enabled" }, extra_body: { x: 1 }, effort: "low", output_config: { o: 2 }, reasoning: "banana" });
  ...
  test("top-level system merged with message systems", () => {
    const out = wrap({ model: "m", system: "explicit", messages: [{ role: "system", content: "from-msg" }, { role: "user", content: "hi" }] });
    expect((out.params as Record<string, any>).system).toBe("from-msg\n\nexplicit");
  });

  test("max_tokens NaN / -1 / 0 omitted", () => {
    expect((wrap({ model: "m", messages: [], max_tokens: NaN }).params as Record<string, any>).max_tokens).toBeUndefined();
    expect((wrap({ model: "m", messages: [], max_tokens: -1 }).params as Record<string, any>).max_tokens).toBeUndefined();
    expect((wrap({ model: "m", messages: [], max_tokens: 0 }).params as Record<string, any>).max_tokens).toBeUndefined();
    expect((wrap({ model: "m", messages: [], max_completion_tokens: 50 }).params as Record<string, any>).max_tokens).toBe(50);
  });

  test("vision: image_url preserved for vision model, stripped for text model", () => {
    const img = [{ type: "text", text: "look" }, { type: "image_url", image_url: { url: "https://x/y.png" } }];
    const vision = wrap({ model: "kimi-k2.7", messages: [{ role: "user", content: img }] });
    expect(vision.params.messages).toEqual([{ role: "user", content: [{ type: "text", text: "look" }, { type: "image", image: "https://x/y.png" }] }]);
  ...
  test("vision: anthropic base64 source block converted", () => {
    ...
    expect(out.params.messages).toEqual([{ role: "user", content: [{ type: "image", image: "data:image/jpeg;base64,AAEC" }] }]);
  });
  ...
  test("call-level name fallback when function.name empty", () => {
    const out = wrap({
      model: "m",
      messages: [
        { role: "assistant", content: "", tool_calls: [{ id: "c1", type: "function", name: "call-lvl", arguments: "{}" }] },
...
describe("command-code reply", () => {
  test("aggregate SSE → chat completion JSON", async () => {
    ...
    const res = await commandCodeReply(new Response(sse, { headers: { "content-type": "text/event-stream" } }), false, "deepseek/deepseek-v4-flash", new Map());
    expect(res.status).toBe(200);
    const body = (await res.json()) as {
      choices: { message: { content: string; reasoning_content: string; tool_calls: { id: string; function: { name: string; arguments: string } }[] }; finish_reason: string }[];
      usage: { prompt_tokens: number; completion_tokens: number; total_tokens: number; prompt_tokens_details: { cached_tokens: number }; completion_tokens_details: { reasoning_tokens: number }; reasoning_tokens: number };
    };
    expect(body.choices[0].message.content).toBe("Hello");
    expect(body.choices[0].message.reasoning_content).toBe("hmm");
    expect(body.choices[0].message.tool_calls[0]).toMatchObject({ id: "call_9", function: { name: "grep", arguments: '{"q":"x"}' } });
  ...
  test("streaming → chat chunks", async () => {
    ...
    const res = await commandCodeReply(new Response(sse, { headers: { "content-type": "text/event-stream" } }), true, "m", new Map());
  ...
  test("error event → 502", async () => {
    ...
    const res = await commandCodeReply(new Response(sse, { headers: { "content-type": "text/event-stream" } }), false, "m", new Map());
